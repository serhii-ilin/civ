# Value Constitution Draft

All framework decisions must satisfy this value constitution unless an iteration explicitly proposes an amendment. Values have stable IDs so they can be referenced, locked, revised, or stress-tested across runs.
...
When values compete, the agent must:
...
5. Define what evidence or simulation result would favor one option.
6. Record the choice in the decision log.

## Amendment Rules

Value amendments require:

- A named change proposal with a stable ID.
- A rationale grounded in evidence, contradiction resolution, or simulation results.
- A compatibility analysis against all existing values.
- An adversarial review from at least three perspectives: skeptic, historian, and behavioral scientist.
- A migration note explaining what existing artifacts must change if the amendment is accepted.
